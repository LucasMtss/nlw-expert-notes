...
export function NoteCard({note, onNoteDeleted}: NoteCardProps){
    ...
    return (
        <Dialog.Root open={open} onOpenChange={setOpen}>
            <Dialog.Trigger className="rounded-md text-left flex-col bg-slate-800 p-5 gap-3 overflow-hidden relative hover:ring-2 focus-visible:ring-2 focus-visible:ring-lime-400 outline-none">
            ...
            <Dialog.Portal>
                ...
                <Dialog.Content className='overflow-hidden fixed inset-0 md:inset-auto md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:h-[60vh] md:max-w-[640px] w-full bg-slate-700 md:rounded-md flex flex-col outline-none'>
                    <Dialog.Close className='absolute right-0 bg-slate-800 p-1.5 text-slate-400'>
                    ...
                    <button onClick={() => handleDeleteNote()} type="button" className='font-medium w-full bg-slate-800 py-4 text-center text-sm text-slate-300 outline-none group'>
